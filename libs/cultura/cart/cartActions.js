...
export const updateCartItemQuantity = async (cartId, itemId, quantity) => {
  ...
  const query = `
    mutation updateCartItemsQuantity($input: UpdateCartItemsInput!) {
  ...
  const variables = {
    input: {
      cart_id: cartId,
      cart_items: [{ cart_item_id: itemId, quantity }],
    },
  };
  ...
  try {
    ...
    const result = await response.json();
    return result.data.updateCartItems;
  } catch (error) {
    console.error("Erreur lors de la récupération des données :", error);
    throw error;
  }
...
export const removeItemFromCart = async (cartId, itemId) => {
  ...
  const query = `
    mutation removeItemFromCart($cartId: String!, $itemId: Int!, $isGift: Boolean) {
    removeItemFromCart(input: { cart_id: $cartId, cart_item_id: $itemId, is_gift: $isGift }) {
        cart {
            id
            cart_error
  ...
  try {
    ...
    const result = await response.json();
    return result.data.removeItemFromCart;
  } catch (error) {
    console.error("Erreur lors de la récupération des données :", error);
    throw error;
  }
