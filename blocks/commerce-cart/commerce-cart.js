...
import CustomCartSummaryList from "./CustomCartSummaryList.js";
...
export default async function decorate(block) {
  ...
  await Promise.all([
    ...
    //   routeProduct: (product) => rootLink(`/products/test/${product.urlKey}`),
    ...
    // Cart List - Utilisation du composant personnalisé
    // eslint-disable-next-line consistent-return
    (() => {
      try {
        return new CustomCartSummaryList($list, {
          routeProduct: (product) =>
            rootLink(`/products/test/${product.urlKey}`),
        });
      } catch (error) {
        console.error(
          "Erreur lors de l'initialisation du CustomCartSummaryList:",
          error
        );
      }
    })(),
